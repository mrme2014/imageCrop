...
public class CropObject {
    ...
    public void setMinInnerSideSize(float minSideWidth, float minSideHeight) {
        if (minSideWidth <= 0 || minSideHeight < 0) {
            throw new IllegalArgumentException("Min dide must be greater than zero");
        }
        mMinSideSize = minSideWidth;
        mMinSideHeight = minSideHeight;
    }
    ...
    public boolean moveCurrentSelection(float dX, float dY) {
        ...
        if (movingEdges == MOVE_BLOCK) {
            ...
            if ((movingEdges & MOVE_LEFT) != 0) {
                dx = Math.min(crop.left + dX, crop.right - mMinSideSize) - crop.left;
            }
            if ((movingEdges & MOVE_TOP) != 0) {
                dy = Math.min(crop.top + dY, crop.bottom - mMinSideHeight) - crop.top;
            }
            if ((movingEdges & MOVE_RIGHT) != 0) {
                dx = Math.max(crop.right + dX, crop.left + mMinSideSize)
                        - crop.right;
            }
            if ((movingEdges & MOVE_BOTTOM) != 0) {
                dy = Math.max(crop.bottom + dY, crop.top + mMinSideHeight)
                        - crop.bottom;
            }
